...
template<typename V, typename I>
void Mesh<V, I>::setVertexBufferData(const V *data, size_t count) {
...
template<typename V, typename I>
void Mesh<V, I>::setInstanceBufferData(const I *data, size_t count) {
    static_assert(!std::is_same_v<I, void>, "Template for instance data must not be void");
...
template<typename V, typename I>
void Mesh<V, I>::setIndexBufferData(const std::vector<unsigned int> &indices) {
    if (vertexVbo == 0) return;
...
template<typename V, typename I>
void Mesh<V, I>::configureVertexAttributes() {
    glBindVertexArray(vao); glCheckError();
    ...
    if (instanceVbo != 0 && instanceAttributes.size() > 0) {
